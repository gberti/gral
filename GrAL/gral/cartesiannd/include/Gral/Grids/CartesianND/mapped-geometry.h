...
namespace cartesiannd {
  ...
  public:
    ...
    mapped_geometry(grid_type const& gg, mapping_type ff = mapping_type()) 
      : g(gg), f(new mapping_type(ff)), low_(gg.low_vertex_index()), high_(gg.high_vertex_index()) { init();}
    mapped_geometry(grid_type const& gg, index_type lw, index_type hgh, 
		    mapping_type ff = mapping_type()) 
      : g(gg), f(new mapping_type(ff)), low_(lw), high_(hgh) { init();}

    void set_inverse_mapping(mapping_type inv) { f_inverse = new mapping_type(inv);}
  ...
  template<class CARTGRID, class MAP>
	...
	coord_type projected_local_p = clamp_tuple(unit_coord(TheGrid()->low_vertex_index()), 
        ...
        // FIXME: Problems occur for points at 'upper' grid boundary (one coordinate = higher grid coordinate)
